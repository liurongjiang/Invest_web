$(document).ready(function() {
    ...
    $.ajax({
      type: "GET",                      //请求类型
      url: "/invest/event/project_json?matrix_id=" + res[1],
    ...
    $.ajax({
        type: "GET",                      //请求类型
        url: "/invest/event/team_list?matrix_id=" + res[1],
        dataType: "json",                 //返回的数据类型
        success: function(data){          //data就是返回的json类型的数据
            var team_info='';
            for(index in data){
    ...
    var table = $('#invest').DataTable({
      ...
      "autoWidth": true,
      "ajax": {
        "url": "/invest/event/event_list?matrix_id=" + res[1],
    ...
    $.ajax({
        url: '/invest/event/log/check_record/' + res[1],
    });
